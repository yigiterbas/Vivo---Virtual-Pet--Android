...
public class ScaleGestureDetector {
    ...
    public boolean onTouchEvent(View view, MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            reset();
            firstx=event.getRawX();// Start fresh
            firsty=view.getY();
        }
        ...
        xpos=lastx-firstx;
        xprevpos = xprevpos+xpos;
        if (mCallback1 != null) {
            float positionx=0;
            float positiony=0;
            if(movement=="Right") positionx=firstx;
            else positionx=view.getX();
            if(ypos>0) positiony=firsty;
            else positiony=view.getY();



            mCallback1.onRectFinished(new Rect((int) view.getX(), (int) view.getY(), (int)view.getWidth(), (int) (view.getHeight())));
        }
        return handled;
    }
